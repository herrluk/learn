...
![chrome-HOWTO](JS 学习笔记.assets/l.jpeg)
...
JavaScript**<u>不区分整数和浮点数</u>**，统一用Number表示，以下都是合法的Number类型：
...
实际上，JavaScript**<u>允许对任意数据类型做比较</u>**：
...
由于JavaScript这个设计缺陷，*不要*使用`==`比较，始终<u>坚持使用`===`比较</u>。
...
在之前的*JavaScript（ES5之前）*中，`var`在像 `if` ，`for`循环中是没有作用域的，只有借助`function`的作用域来解决引用作用域外面的变量的问题。

在*ES6*中加入了`let`，它在`if`和`for`中是有块级作用域的，let拥有块级作用域,一个`{}`就是一个作用域，也就是`let`声明块级变量，即局部变量。

`const` 也是有块级作用域的，当我们的修饰的标识符不想再被改变赋值时用`const`。

**建议** *在ES6开发中优先使用`const`，只有想改变一个标识符的时候才使用`let`*

**注意**

- const赋值后不能修改
- 用const定义标识符必须赋值
- 常量的定义是指向的对象不能修改，但是对象的属性可以修改
...
`for`循环的一个*变体是`for ... in`循环，它可以把一个对象的所有属性依次循环出来：*
